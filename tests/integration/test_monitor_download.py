...
class TestMonitorDownloadIntegration(unittest.TestCase):
    ...
    def test_monitor_with_download_flag(self, mock_get):
        ...
        sys.argv = ['github_monitor.py', '--config', str(self.test_config), '--download']

        with patch('sys.stdout') as mock_stdout:
            monitor_main()

        # Verify download directory was created
        download_dir = Path(self.temp_dir) / "downloads" / "kubernetes" / "kubernetes" / "v1.25.0"
    ...
    def test_monitor_download_only_new_versions(self, mock_get):
        ...
        v125_dir = Path(self.temp_dir) / "downloads" / "kubernetes" / "kubernetes" / "v1.25.0"
        v124_dir = Path(self.temp_dir) / "downloads" / "kubernetes" / "kubernetes" / "v1.24.0"
    ...
    def test_monitor_download_error_handling(self, mock_get):
        ...
        mock_api_response.json.return_value = [self.mock_github_response]
